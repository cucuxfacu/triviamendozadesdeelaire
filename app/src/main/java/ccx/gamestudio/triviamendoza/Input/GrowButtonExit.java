...
public abstract class GrowButtonExit extends Sprite {
	...
	// CONSTRUCTOR
	// ====================================================
	public GrowButtonExit(final float pX, final float pY, final ITextureRegion pTextureRegion) {
	...
	public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		if (pSceneTouchEvent.getAction() == TouchEvent.ACTION_DOWN) {
			...
			mIsClicked = true;
			mTouchStartedOnThis = false;
			SFXManager.playCloseLayer(1f, 0.5f);
		}
		return true;
	}
}